...
public class PhieuDichVuController implements ActionListener, Controller {
    ...
    private void checkCustomer() {
        ...
        if (s.length() != 0) {
            NguoiDTO nguoi = mPhieuDichVuModel.kiemtraKhachQuen(Integer
                    .valueOf(s));
            if (nguoi != null) {
                ...
                JOptionPane
                        .showMessageDialog(null, "Bạn không phải khách quen");
            }
        } else {
    ...
    private void saveData() {
        ...
        if (this.isTableEmpty() != true
                && mPhieuDichVuView.isAllTextFilled() == true
                && mPhieuDichVuView.getDateNgayDK().getDate().getTime()<= mPhieuDichVuView
                .getDateNgayGiao().getDate().getTime()
                && CheckInput.isStringMax50(mPhieuDichVuView.getTextHoTen()
                        .getText())
                && CheckInput.isStringMax50(mPhieuDichVuView.getTextMaKH()
                        .getText())
                && CheckInput.isStringMax300(mPhieuDichVuView.getTextDiaChi()
                        .getText())) {
            double finalMoney = calculateFinalMoney();
            mPhieuDichVuView.getTextTongCong().setText(finalMoney + "");
            String shortId = mPhieuDichVuView.getTextMaKH().getText()
                    .toString().trim();
            NguoiDTO nguoi = mPhieuDichVuModel.kiemtraKhachQuen(Integer
                    .valueOf(shortId)); //
            // kiểm tra ở đây làm gì ta? khi ấn nút kiểm tra => rồi laod thông
            // tin khách hàng.
            int maKH = 0;
            if (nguoi == null) {
                ...
                NguoiDTO newNguoi = new NguoiDTO(nextNguoiId, mPhieuDichVuView
                        .getTextHoTen().getText().toString().trim(),
                        mPhieuDichVuView.getTextDiaChi().getText().toString()
                        .trim(), Integer.parseInt(mPhieuDichVuView
                                .getTextMaKH().getText().toString().trim()));
                KhachHangDTO kh = new KhachHangDTO(maKH, nextNguoiId, false);

                mPhieuDichVuModel.insertKhachHang(kh, newNguoi);
            } else {
                maKH = mPhieuDichVuModel.getMaKhByName(Integer
                        .parseInt(shortId));
            }

            int nextIdPhieuThu = mPhieuDichVuModel.getNextIdOfPhieuThu();
            P_ThuDTO pThu = new P_ThuDTO(nextIdPhieuThu, maKH, new Timestamp(
                    mPhieuDichVuView.getDateNgayDK().getDate().getTime()),
                    new Timestamp(mPhieuDichVuView.getDateNgayGiao().getDate()
                            .getTime()), finalMoney);

            mPhieuDichVuModel.insertP_Thu(pThu);

            int nextIdPhieuDichVu = Integer.parseInt(mPhieuDichVuView
                    .getTextMaPhieu().getText());
            P_DichVuDTO pDichVu = new P_DichVuDTO(nextIdPhieuDichVu,
                    nextIdPhieuThu);
            mPhieuDichVuModel.insertP_DichVu(pDichVu);

            for (TableData data : mPhieuDichVuView.getmTableModel()
                    .getmObjectList()) {
                ...
                CTP_DichVuDTO ctp = new CTP_DichVuDTO(
                        mPhieuDichVuModel.getNextIdOfCTPDV(),
                        nextIdPhieuDichVu, idDv, Integer.parseInt(data
                ...
                clearData();
                addMoreTableRecord();
                mPhieuDichVuView.getTextMaPhieu().setText(
                        String.valueOf(mPhieuDichVuModel
                                .getNextIdOfPhieuDichVu()));
            }
        } else {
            JOptionPane.showMessageDialog(null,
                    "Vui lòng kiểm tra các thông tin sau:\n"
                    + "- Ngày giao phải lớn hơn ngày đăng ký\n"
                    + "- Các loại mã, họ tên, địa chỉ,... phải hợp lệ.");
        }
    ...
    private void createView() {
        ...
        JComboBox<String> combo = new JComboBox<String>(mDichVu);
        mPhieuDichVuView.getmTable().getColumnModel().getColumn(0)
                .setCellEditor(new DefaultCellEditor(combo));
        mPhieuDichVuView.getmTable().putClientProperty(
        ...
        combo.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                for (int i = 0; i < mPhieuDichVuModel.getAllDichVu().size(); i++) {
                    if (mPhieuDichVuModel.getAllDichVu().get(i).getTenDV()
                            .equals(tenDichVu)) {
                        dongia = mPhieuDichVuModel.getAllDichVu().get(i)
                                .getDonGia();
                        break;
                    }
                }
                ...
                if (selectedRow > -1) {
                    mPhieuDichVuView.getmTableModel().setValueAt(dongia + "",
                            selectedRow, 2);
                    if (mPhieuDichVuView.getmTableModel()
                            .getValueAt(selectedRow, 1).toString().trim()
                            .length() != 0) {
                        mPhieuDichVuView.getmTableModel().setValueAt(
                                dongia
                                * Integer.valueOf(mPhieuDichVuView
                                        .getmTableModel()
                                        .getmObjectList()
        ...
        mPhieuDichVuView.getmTable().getColumnModel().getColumn(1)
                    ...
                    if (isInteger(textField.getText().toString().trim())) {
                        mPhieuDichVuView.getmTableModel().setValueAt(
                                ...
                                * Integer.parseInt(textField.getText()
                                        .toString().trim()) + "",
                                mPhieuDichVuView.getmTable().getSelectedRow(),
                                3);
                    } else {
        ...
        mPhieuDichVuView.getTextMaPhieu().setText(
                String.valueOf(mPhieuDichVuModel.getNextIdOfPhieuDichVu()));
    ...
    private void addMoreTableRecord() {
        ...
        if ((size > 0 && mPhieuDichVuView.getmTableModel()
    ...
    private void removeTableRecord() {
        if (mPhieuDichVuView.getmTable().getSelectedRow() >= 0) {
            mPhieuDichVuView.getmTableModel().removeObject(
                    mPhieuDichVuView.getmTable().getSelectedRow());
        } else {
    ...
    private double calculateFinalMoney() {
        ...
        for (int i = 0; i < mPhieuDichVuView.getmTable().getRowCount(); i++) {
            sum += Double.parseDouble((String) mPhieuDichVuView
    ...
    private boolean isTableEmpty() {
        for (int i = 0; i < mPhieuDichVuView.getmTable().getRowCount(); i++) {
            if (mPhieuDichVuView.getmTableModel().getValueAt(i, 3).toString()
                    .trim().length() == 0) {
                return true;
            }
        }
    ...
    @Override
    public void dipose() {
        mPhieuDichVuView.getFrame().dispatchEvent(
                new WindowEvent(mPhieuDichVuView.getFrame(),
